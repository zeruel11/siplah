...
<form class="needs-validation" id="input" name="input" method="post" action="<?= ($mode=="insert")?base_url('index.php/Beranda/tambahRenovasi'):base_url('index.php/Beranda/ubahRenovasi/').$dataRenovasi[0]['idProposal'] ?>" novalidate>
	<div class="form-group">
		...
		<input type="text" class="form-control<?= (form_error('judulProposalForm'))?' is-invalid':(set_value('judulProposalForm')?' is-valid':'') ?>" id="judulProposalForm" name="judulProposalForm" placeholder="Masukkan judul renovasi" value="<?= ($mode=="insert")?"":(set_value('judulProposalForm')==NULL)?$dataRenovasi[0]['judulProposal']:set_value('judulProposalForm') ?>"></input>
